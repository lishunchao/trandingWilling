...
重复运行 `start_web.ps1` 会自动关闭该端口上的旧版交易控制台并加载最新代码；如果端口由其他程序占用，脚本会安全停止并提示，不会误关无关程序。
...
- `GET /api/v1/marks?symbols=BTCUSDT,ETHUSDT`：批量读取 Binance USDT 永续标记价格。
- `GET /api/v1/dynamic-stops`：按最新完成的 15 分钟结构与 0.4 ATR 缓冲计算动态止损。
...
页面默认优先展示增强版。信号区的“增强版 / 基准版 / 全部”按钮会同时过滤当前信号和纸面仓位，并显示各版本的当前信号数量；过滤仅作用于页面展示。

交易所标记价格每 5 秒刷新。未成交信号显示实时参考价与原触发价；纸面仓位的成交入场价保持不可变，同时显示实时价。动态结构止损在每个 15 分钟收盘后约 7 秒检查一次，按最近五根已完成 K 线结构与 0.4 ATR 缓冲重新计算，只允许收紧、不允许放宽；失败时每 15 秒重试，最多 3 次。页面继续每 5 秒计算当前标记价格距动态止损的百分比。
